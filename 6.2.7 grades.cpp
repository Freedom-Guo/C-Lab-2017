...
int main()
{
	...
	//输入数据
	printf("Input 5 classes' name (single letters only):\n");
	for (i = 0; i < M; i++)
	{
		scanf("%c", className + i);
		getchar();
	...
	printf("Please input each student's name followed by \nhis (or her) scores of classes:\n");
	for (j = 0; j < N; j++)
	{
	...
	for (i = 0; i < N; i++)
	{
		...
		for (j = 0; j < M; j++)
			if (*(*(studentGrade + j) + i) < 60.0)
				num++;
		printf("Number of students fail %c is %d\n", *(className + i), num);
	}
